...
def cluster_accuracy(y_true, y_pre,y_pred_full, return_aligned=False):
    ...
    y_best_full = np.zeros(y_pred_full.shape)
    for i in range(nClass2):
        y_best[y_pre == Label2[i]] = Label1[c[i]]
        y_best_full[y_pred_full == Label2[i]] = Label1[c[i]]
    # # calculate accuracyi
    ...
    bcubed_precision_, bcubed_recall_, bcubed_F = bcubed(y_true, y_best, cm=None)
    pur = purity_score(y_true, y_best)
    if return_aligned:
        return y_best_full, acc, kappa, nmi, ari, pur, bcubed_F, ca
    return acc, kappa, nmi, ari, pur, bcubed_F, ca
